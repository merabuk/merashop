...
class IntegerAttributeValueProvider implements ProductAttributeValueProviderInterface
{
    ...
    public function handle(Attribute $attribute, ProductAttributeValueData $data): array
    {
        ...
        if (false === $valueData instanceof IntegerAttributeValueData) {
            throw $this->makeInvalidValueDataException(actualClass: $valueData::class, expectedClass: IntegerAttributeValueData::class);
        }

        return [
            ProductAttributeValue::createWithValue(
                attributeId: AttributeId::fromInt($data->attributeId),
                value: IntegerValue::fromInt($valueData->value),
            ),
        ];
    }
