...
class FloatAttributeValueProvider implements ProductAttributeValueProviderInterface
{
    ...
    public function handle(Attribute $attribute, ProductAttributeValueData $data): array
    {
        ...
        if (false === $valueData instanceof FloatAttributeValueData) {
            throw $this->makeInvalidValueDataException(actualClass: $valueData::class, expectedClass: FloatAttributeValueData::class);
        }

        return [
            ProductAttributeValue::createWithValue(
                attributeId: AttributeId::fromInt($data->attributeId),
                value: FloatValue::fromFloat($valueData->value),
            ),
        ];
    }
